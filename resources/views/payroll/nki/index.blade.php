...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div class="min-w-0 flex-1">
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">NKI (Tunjangan Prestasi)</h1>
            <p class="mt-1 text-xs sm:text-sm text-gray-600">Performance rating: Kemampuan (20%), Kontribusi 1 (20%), Kontribusi 2 (40%), Kedisiplinan (20%)</p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            ...
            @if(auth()->user()->hasPermission('nki.import'))
            ...
            <a href="{{ route('payroll.nki.create') }}" 
               class="px-3 py-2 text-xs sm:text-sm bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium rounded-lg hover:from-indigo-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 whitespace-nowrap">
                <i class="fas fa-plus mr-1.5"></i>Add NKI
            </a>
            @endif
        </div>
    </div>
    ...
    <!-- NKI Table with Real-Time Auto-Refresh -->
