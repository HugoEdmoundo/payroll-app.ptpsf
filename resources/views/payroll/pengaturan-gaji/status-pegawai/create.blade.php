...
@section('title', 'Add Pengaturan Gaji Status Pegawai')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Add Pengaturan Gaji Status Pegawai</h1>
            <p class="mt-1 text-sm text-gray-600">Create new salary configuration for employee status</p>
        </div>
    ...
    <!-- Form -->
    <div class="card p-6">
        <form action="{{ route('payroll.pengaturan-gaji.status-pegawai.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="lokasi_kerja" required
                        ...
                        @foreach($settings['lokasi_kerja'] as $key => $value)
                            <option value="{{ $value }}" {{ old('lokasi_kerja') == $value ? 'selected' : '' }}>
                                {{ $value }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" 
                              rows="3"
                              class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                              placeholder="Optional notes...">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
